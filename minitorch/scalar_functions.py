...
class Mul(ScalarFunction):
    ...
    def backward(ctx: Context, d_output: float) -> Tuple[float, float]:
        ...
        return float(operators.mul(d_output, b)), float(operators.mul(d_output, a))
...
class Inv(ScalarFunction):
    ...
    def backward(ctx: Context, d_output: float) -> float:
        (a,) = ctx.saved_values
        inv_a = operators.inv(a)
        return float(
            operators.mul(d_output, operators.neg(operators.mul(inv_a, inv_a)))
        )
...
class ReLU(ScalarFunction):
    ...
    def backward(ctx: Context, d_output: float) -> float:
        (a,) = ctx.saved_values
        return float(d_output if a > 0.0 else 0.0)
...
class Exp(ScalarFunction):
    ...
    def backward(ctx: Context, d_output: float) -> float:
        (e,) = ctx.saved_values
        return float(operators.mul(d_output, e))
